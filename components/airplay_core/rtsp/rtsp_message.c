...
static int parse_decimal_header_value(const char *value, int fallback) {
  if (!value) {
    return fallback;
  }
...
static uint16_t parse_transport_port_value(const char *value) {
  int parsed = parse_decimal_header_value(value, 0);
...
int rtsp_parse_cseq(const char *request) {
  ...
  if (cseq) {
    return parse_decimal_header_value(cseq + 5, 1);
  }
  return 1;
}
...
int rtsp_parse_content_length(const char *request) {
  ...
  if (cl) {
    int parsed = parse_decimal_header_value(cl + 15, 0);
...
void rtsp_parse_transport(const char *request, uint16_t *control_port,
  ...
  if (cp && cp < line_end && control_port) {
    *control_port = parse_transport_port_value(cp + 13);
  }

  // Parse timing_port
  const char *tp = strstr(transport, "timing_port=");
  if (tp && tp < line_end && timing_port) {
    *timing_port = parse_transport_port_value(tp + 12);
  }
}

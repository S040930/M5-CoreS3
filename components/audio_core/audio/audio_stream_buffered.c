...
#else
#define AUDIO_BUFFERED_TASK_CORE 1
#endif
...
static esp_err_t buffered_start(audio_stream_t *stream, uint16_t port) {
  ...
  // This avoids competing with networking and control tasks for internal DRAM.
  if (!s_buffered_stack) {
  ...
  state->buffered_task_handle =
      xTaskCreateStaticPinnedToCore(
          ...
          5, s_buffered_stack, &s_buffered_tcb, AUDIO_BUFFERED_TASK_CORE);
...
static const audio_stream_ops_t s_audio_stream_buffered_ops = {
...
audio_stream_t *audio_stream_create_buffered(void) {
  ...
  stream->ops = &s_audio_stream_buffered_ops;

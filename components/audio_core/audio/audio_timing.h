...
typedef struct {
  ...
  bool playing;
  bool anchor_valid;
  bool control_sync_seen;
  bool ntp_offset_sane;
  ...
  // several seconds).  Cleared when timing is on-time, or after
  // POST_FLUSH_MAX_DWELL_US as a safety cap — not after a short timeout while
  ...
  int64_t anchor_projection_offset_ns;
  int64_t last_clock_state_log_us;
} audio_timing_t;

typedef enum {
  AUDIO_ANCHOR_SOURCE_NONE = 0,
  AUDIO_ANCHOR_SOURCE_LOCAL,
  AUDIO_ANCHOR_SOURCE_NTP,
  AUDIO_ANCHOR_SOURCE_PTP,
} audio_anchor_source_t;

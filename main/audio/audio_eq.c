...
static const float k_default_harman_like[SETTINGS_EQ_BANDS] = {
...
static float avg_range(const float gains[SETTINGS_EQ_BANDS], int start, int end) {
  float sum = 0.0f;
  int n = 0;
...
static void update_coeffs_from_15band(const float gains[SETTINGS_EQ_BANDS]) {
...
esp_err_t audio_eq_init(uint32_t sample_rate_hz) {
  ...
  if (settings_get_eq_gains(gains) == ESP_OK) {
    update_coeffs_from_15band(gains);
  } else {
